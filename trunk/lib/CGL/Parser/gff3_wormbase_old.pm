#-------------------------------------------------------------------------------
#------                          CGL::Parser::gff3_wormbase              -------
#-------------------------------------------------------------------------------
package CGL::Parser::gff3_wormbase_old;
use strict;
...
sub new {
        ...
        my ($genes, $seq, $seq_id) =
        get_genes($gff_file, $fasta_file, 'exon', 0);
...
sub to_gff3_gene {
	...
	my $g_id   = $g->{id};
	my $g_n;
	$g_n = $g->{f}->get_SeqFeatures('Name')->value();
...
sub to_gff3_exon {
        ...
        my $t_id   = $t->{id};
        my $e_id   = $e->{id};
	my $g_id   = $t->{f}->get_SeqFeatures('Parent')->value();
...
sub split_file {
	...
	my $i = 0;
        foreach my $g (sort @{$genes}){
                ...
                if ($ds_root) {
                        $ds->mkdir($file_base) || 
			    die "Unable to ds->mkdir() for $file_base";
                ...
                my $seg_id;
                if ($g->{f}->strand() == 1){
                        $seg_id = join ':', ($seq_id, $g->{src_s}, $g->{src_e});
                }
                else {
                        $seg_id = join ':', ($seq_id, $g->{src_e}, $g->{src_s});
                }

                print $fh to_gff3_contig($seg_id, length($g->{seq}))."\n";

                print $fh to_gff3_gene($seg_id, $g)."\n";
                foreach my $t (sort @{$g->{mRNAs}}){
...
sub load_transcript {
	...
	my $status = $t->{f}->get_SeqFeatures('prediction_status')  ?
	    $t->{f}->get_SeqFeatures('prediction_status')->value()  :
	    undef;
...
sub get_valid_exons {
	...
	my $g_id = 
	    $exons->[0]->{f}->get_SeqFeatures('Parent') ?
	    $exons->[0]->{f}->get_SeqFeatures('Parent')->value() :
	    undef;

	my $t_id = 
	    $cdss->[0]->{f}->get_SeqFeatures('Parent') ?
	    $cdss->[0]->{f}->get_SeqFeatures('Parent')->value() :
	    undef;
